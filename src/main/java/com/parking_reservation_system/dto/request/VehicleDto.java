...
public record VehicleDto(
        ...
        @NotBlank(message = "Plate number is required")
                ...
                String plateNumber,
        @Positive(message = "Model year must be a positive number") int modelYear,
        @NotBlank(message = "Model name is required") String modelName,
        @Positive(message = "Vehicle width must be positive") double vehicleWidth,
        @Positive(message = "Vehicle depth must be positive") double vehicleDepth,
        @NotNull(message = "Vehicle type must be specified") @Enumerated(EnumType.STRING)
                VehicleType type,
        @NotNull(message = "a vehicle should be tied to user") Integer user_id) {}
