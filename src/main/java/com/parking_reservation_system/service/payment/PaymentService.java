    ...
    */
    ...
    public String initiateCardPayment(int reservationId, UUID idempotencyKey) {
        ...
        if (recordOpt.isPresent()) {
            ...
            if ("COMPLETED".equals(record.getStatus())) return record.getResponse_body();
        ...
        if (reservation.getStatus() != Reservation.Status.PENDING) {
            throw new RuntimeException(
                    "CRITICAL: Reservation " + reservationId + " is not pending");
        }
                        ...
                        record -> {
                            ...
                            if (!isZombie && "PROCESSING".equals(record.getStatus())) {
                                throw new ResponseStatusException(
                                        HttpStatus.CONFLICT, "Payment already processing");
                            }
    ...
    protected String createPaymentOrder(
            String token, BigDecimal price, int reservationId, UUID key) {
        try {
            int amountCents = price.multiply(BigDecimal.valueOf(100)).intValueExact();
            ...
            oneItem.put(
                    "description",
                    "reservationId_" + reservationId + "," + "Idempotencykey_" + key);
            items.put(oneItem);
